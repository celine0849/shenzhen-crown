...
(function () {
  ...
  // 加载 CloudBase Web SDK（本地文件，随 GitHub Pages 一起部署，大陆原生可达，无需任何外部 CDN）
  ...
  async function ensureCloudbaseSDK() {
    ...
    // 仅加载本地打包文件（assets/tcb.min.js = @cloudbase/js-sdk v3 浏览器版）
    try {
      await loadScript("assets/tcb.min.js");
      if (window.cloudbase) { console.log("[tcb] ✅ SDK 加载成功 (本地 assets/tcb.min.js)"); return true; }
    } catch (e) { console.warn("[tcb] ❌ 本地 SDK 加载失败:", e.message); }
    console.error("[tcb] 💥 本地 SDK 加载失败，CloudBase 不可用");
    return false;
  }
